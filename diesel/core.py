    ...
    def check_child_connection(self, remote_addr):
        ...
        if not conn_key in self.udp_connections:
            if True: # self.owner.should_start_new_connection(remote_addr, datagram):
                client_connection = ReliableUDPClientConnection(self, self.sock, remote_addr)
...
class ReliableHeader(object):
    import struct
    #struct = struct.Struct("I I I")
    struct = struct.Struct("I i")
...
#    def __init__(self, parent, sock, ip=None, port=None, f_connection_loop = None, *args, **kw ):
        #super(ReliableUDPClientConnection, self).__init__(parent, sock, ip, port)
    ...
    def process_datagram(self, dgram):
        #TODO: remove the header bytes that we have already processed
